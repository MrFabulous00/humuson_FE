<template>
	...
	>
		...
		>
			...
			<transition :css="false" @enter="tEnter" @enter-cancelled="tEnterCancelled" @leave="tLeave" @leave-cancelled="tLeaveCancelled">
...
<script>
	...
	export default {
		...
		computed: {
			...mapState(['vxSidebarMainAccordionMode', 'vxSidebarMainScrollToActive', 'vxActiveLang']),
			menuItems() {
		...
		created() {
			...
			// })
			this.menuItems.forEach(k => {
				// if (typeof k.level !== 'undefined' && k.level.indexOf(String(this.$auth.user.level)) === -1) {
				// 		k.hide = true
				// }
				if (typeof k.submenu !== 'undefined') {
					this.setSectionOpen(k)
