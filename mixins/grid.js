export default {
	...
	//     const grid = elem || this.$refs.grid;
	//     if (
	//       event.instance.store.column.allColumnMap[event.columnName]
	//         .sortingType !== "asc"
	//     ) {
	...
	//       grid.invoke(
	//         "focus",
	//         grid.invoke("validate")[0].rowKey,
	//         grid.invoke("validate")[0].errors[0].columnName,
	//         true
	//       );
	//       alert("다시 입력해주세요.");
	//       return;
	//     }
	//     for (var i in this.checked) {
	//       let { zoneNm, zoneTypeCd, zoneAttrCd, zoneDesc, id } = grid.invoke(
	//         "getRow",
	//         this.checked[i]
	//       );
	//       gridData = {
	...
	//     } else if (
	//       columnArr.filter(item => item === focusRow.columnName).length
	//     ) {
